...
const CostHistoryFoundation = ({ history, setHistory }) => {
  const { addPondasiEstimate, removePondasiEstimate } = useBudget(); // Menggunakan addPondasiEstimate dan removePondasiEstimate
  ...
  const handleDeleteEstimate = (index) => {
    removePondasiEstimate(index); // Menggunakan removePondasiEstimate
    setHistory((prevHistory) => prevHistory.filter((_, i) => i !== index));
  };

  const handleAddEstimate = (estimate) => {
    addPondasiEstimate(estimate); // Menggunakan addPondasiEstimate
    Swal.fire({
      icon: "success",
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md">
      <h2 className="mb-6 text-xl font-bold">
        History of Foundation Estimates
      </h2>
      <ul>
